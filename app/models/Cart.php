...
class Cart extends AppModel
{
    public function addToCart($product, $qty = 1, $mod = null)
    {
        ...
        $_SESSION['cart.sum'] = round($_SESSION['cart.sum']);
    }

    public function changeQtyCart($product, $qty, $mod = null)
    {
        if (!isset($_SESSION['cart.currency'])){
            $_SESSION['cart.currency'] = App::$app->getProperty('currency');
        }
        ...
        $_SESSION['cart.qty'] = isset($_SESSION['cart.qty']) ?
            $_SESSION['cart.qty'] + $res : $qty;
        $_SESSION['cart.sum'] = isset($_SESSION['cart.sum']) ?
            $_SESSION['cart.sum'] + $res*($price * $_SESSION['cart.currency']['value']):
            $qty * ($price * $_SESSION['cart.currency']['value']);
        $_SESSION['cart.sum'] = round($_SESSION['cart.sum']);
    }
